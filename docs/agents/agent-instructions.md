...
- **mandatory**: Follow the explicit configured review policy without re-enabling an opted-out provider. CodeRabbit is an optional choice, never a mandatory dependency. With `codex`, `claude`, or `coderabbit` enabled, use the supported pre-PR review path and block on failed, unavailable, incomplete, absent, or stale review evidence. Such an outcome never selects `none` automatically. With explicit `none`, do not request a review or wait for reviewer readiness; retain QA and required checks and report the intentional omission. Configuration or runtime support that has not been implemented must not be presented as a working command.
