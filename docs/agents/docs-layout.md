...
```yaml
---
status: pending # pending | parcial | done
created_at: YYYY-MM-DD
updated_at: YYYY-MM-DD
---
```

- `pending`: the findings have not been fully triaged.
- `parcial`: some findings have been routed or resolved, but open work remains.
- `done`: every finding has been resolved, rejected, or routed to a durable
  Spec, ADR, upstream report, or other named destination.
...
- When a finding becomes a Spec, add the pointer in place.
