...
models. AGENTS.md enforces this as a hard rule. Command contracts (flags, outcomes,
monitoring) live in the `roundfix` skill; this doc fixes the role split, the runtime routing,
and how the orchestrator authors Specs.
...
1. `.roundfixrc.yml` pins `defaults.agent: codex` and keeps `defaults.model: ""` — empty means
   the runtime chain selects the model.
