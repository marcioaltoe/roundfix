...
Only the four governed paths are declared. `make baseline-digests` also rewrites
`internal/baseline/preservation_test.go` and eight files under
`internal/baseline/testdata/`, but the changed-path audit does not govern those,
so declaring them would widen this record for no purpose. An earlier revision of
this block listed all twelve; the eight ungoverned entries were removed once the
audit's governed set was measured rather than inferred from a stale checker.
