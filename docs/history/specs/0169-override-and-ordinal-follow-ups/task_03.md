...
2. MUST verify through the built binary that an override of a failed QA Task
   with a `pass` report records the QA Task's status, and a completed QA Task
   records none.
3. MUST verify that both skills, their mirrors and the guide carry the current
   refusal rule and name the command as the only override path.
4. MUST verify that a same-Spec duplicate ordinal is reported and that a Spec
   without `_tasks.md` lists the skip.
...
- Date: `2026-09-25`
- QA Report: `qa/qa-report-2026-09-25.md`
