...
2. MUST verify that a Spec with no Task Graph and a recorded supersession
   archives, and that the same Spec without one still refuses.
3. MUST verify that every refusal the command declares is exercised and leaves
   the Spec directory byte-identical.
4. MUST verify that a Spec with a Task Graph keeps archive's existing behavior,
   with and without a supersession present.
5. MUST verify that the amendment adds a file and changes no other, and that an
   archived folder's files are byte-identical.
6. MUST verify that the shipped skill and its mirror describe the command and
   the changed precondition, and that every Task commit stayed inside the
