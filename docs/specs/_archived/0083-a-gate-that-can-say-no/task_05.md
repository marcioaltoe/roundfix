...
- `(git diff --name-only HEAD; git ls-files --others --exclude-standard) | grep -v -E '^(internal/agent/acpx_runner_test\.go|docs/specs/0083-a-gate-that-can-say-no/task_05\.md)$' | grep . ; test $? -eq 1` — expected: exits 0, proving no path outside the declared boundary changed.
