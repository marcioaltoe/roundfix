...
The gate ran twice and did not close. Archived under `qa_override` on the
maintainer's standing decision for this stretch: one gate execution, and if it
does not close, override and move on.

Both executions found real defects, and the cheap ones were fixed rather than
carried:

- F-004: `Negative Control` was coined by Task 05 and never documented. It is in
  `CONTEXT.md` now, alongside `verification_vacuous` and `verification_unknown`,
  which the same contract caught earlier in the day.
- F-003: the coverage record was stale against 27 test additions. Re-recorded
  with the sanctioned command.
- F-006: the gate committed nine scratch repositories as Gitlinks and six
  executables — `roundfix`, `codex-acp` and `acpx`, twice each — into this
  Spec's evidence directory. Removed. The gate caught the Gitlinks and missed
  the binaries, and the defect recurs for any Spec whose acceptance needs a real
  Run, so it is filed at
  `docs/backlog/2026-08-09-the-qa-gate-commits-scratch-repositories-and-binaries.md`.

What stays unproven is F-005: negative-control health stops at parsing. Task 05
shipped the section and the parse; it does not record which controls were
exercised beside a verdict, which is what makes gate health a fact rather than
an assumption. That is a real gap in this Spec's third core feature, and it is
recorded here rather than repaired, because repairing it is implementation work
this Spec's remaining budget was spent on.

The feature itself is proven by use rather than by this gate. The pre-work probe
refused this Spec's own corrective Task on its first dispatch — a guard for an
invariant that holds before and after the work — at a cost of zero Agent turns,
which is exactly the transaction the Spec exists to make.
