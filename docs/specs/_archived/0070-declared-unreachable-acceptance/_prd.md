...
- Active ADR obligations: applicable — ADR-0080 owns verdict semantics and the
  ...
  QA gate as a typed Task node, which is the node whose report this Spec
  ...
  detection boundary, a different subsystem from QA verdict semantics.
  Source: `docs/agents/domain.md`.
- Tooling authority: applicable — express maintainer authorization: on
  2026-08-04 the maintainer authorized the QA gate skill for this Spec,
  recorded at
  `docs/workflow/authorizations/2026-08-04-queue-tail-tooling.md`, whose
  Spec 0070 addendum names it; bounded files: `.agents/skills/qa-gate/**` and
  its `skills/qa-gate/**` mirror, regenerated by `make skills-sync`. The
  blocked-cause taxonomy, verdict rules, and `rows_blocked_*` fields live in
  that skill, so Core Features 2, 3, 7 and 8 cannot be delivered by product
  code alone. Deterministic digest fallout is sanctioned by ADR-0081. Source:
  `docs/agents/agent-instructions.md`.
