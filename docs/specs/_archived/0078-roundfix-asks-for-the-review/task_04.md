...
- `rtk ruby -e 'require "open3"; preimage, stderr, status = Open3.capture3("git", "show", "HEAD:.coderabbit.yaml"); raise ".coderabbit.yaml: preimage unavailable: #{stderr.strip}" unless status.success?; actual = File.binread(".coderabbit.yaml"); marker = "reviews:\n"; raise ".coderabbit.yaml: expected one reviews mapping" unless preimage.scan(marker).length == 1; expected = preimage.sub(marker, marker + "  auto_review:\n    enabled: false\n    description_keyword: \"coderabbit:review\"\n"); raise ".coderabbit.yaml: exact content mismatch" unless actual == expected'`
  — expected: exit 0; relative to the Task's `HEAD` preimage, the manual-review
  block is the first `reviews` entry and every other byte is unchanged.
