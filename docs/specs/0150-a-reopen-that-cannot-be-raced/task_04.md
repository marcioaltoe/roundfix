...
2. MUST verify that a reopen whose gate stopped being stale between preflight
   and the write refuses with exit 2 and leaves the QA Task byte-identical.
3. MUST verify that a reopen through a symlinked Task path rewrites the target
   and leaves the path a symlink.
4. MUST verify that every refusal and success Spec 0149 shipped is unchanged,
   including both confinement checks and the read-only active-Run guard.
5. MUST verify that the shipped skill and its mirror name the recheck refusal,
   and that every Task commit stayed inside the approved authority record.
