...
status: proposed
granted: null
action: document repository-consistent Run naming, identity migration, and evidence-preserving reconciliation
...
# Proposed authority for Spec 0125
...
Only the current command skill and its generated shipped copy are protected paths in this proposal. Repository identity, branch creation, persistence, and regression tests are ordinary source and belong to the later Implementation Design. No .roundfixrc.yml, tool pin, broad directory, cleanup script, or destructive migration command is authorized.
