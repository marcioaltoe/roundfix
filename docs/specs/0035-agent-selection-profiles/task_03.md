...
- [ ] Encode the versioned recommendation schema and fixtures.
- [ ] Add stable category and rank ordering.
- [ ] Resolve optional-category recommendation inheritance.
- [ ] Implement `profiles show` text output.
- [ ] Implement `roundfix/profiles/v1` JSON output.
- [ ] Add no-mutation and exactly-five contract tests.

## Acceptance Criteria

- [ ] Every requested category renders exactly five unique official models in the documented order.
- [ ] Optional categories label themselves while reporting `general` as the recommendation source.
- [ ] Text and JSON expose the same effective profile, fallback order, snapshot evidence, and caveat.
- [ ] Repeated output is byte-stable and independent of map iteration.
- [ ] Showing one or all categories leaves User Config, Project Config, runtime-owned config, recommendation data, and Run state unchanged.
- [ ] No product path reads ranking data to choose a Preferred Selection or Fallback Chain.
