...
- [ ] Add value-selected domain templates for both supported layouts.
- [ ] Implement declared token binding and safe Markdown inline rendering.
- [ ] Bind backend runtime, design runtime, and Verification values to the
      autonomous guide.
- [ ] Resolve the English language policy through the Decision Plan.
- [ ] Derive expected digests and semantic audit from rendered artifacts.
- [ ] Add non-default, unsafe-input, drift, and audit/apply agreement tests.

## Acceptance Criteria

- [ ] Single-context and multi-context answers produce distinct approved domain
      guidance and a clean audit for the selected value.
- [ ] The exact stored backend runtime, design runtime, and Verification values
      appear in active autonomous guidance and its rendered digest inventory.
- [ ] Newline, control-character, or ownership-marker injection returns invalid
      input, performs no write, and names the unsafe decision.
- [ ] Values containing Markdown backticks render without breaking out of their
      inline-code boundary.
- [ ] Editing rendered decision content causes audit to report drift and preview
      the same refresh that apply performs.
- [ ] The QA-07 alternate values pass every rendering and omission assertion,
