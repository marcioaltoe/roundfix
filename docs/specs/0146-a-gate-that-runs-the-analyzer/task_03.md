...
1. MUST run the repository Verification and record its result as a gate fact,
   noting that it now includes the analyzer.
2. MUST verify that the analyzer fails over the control's fixture, naming its
   diagnostic, and that the module-wide run stays silent.
3. MUST verify that the repository Verification composition contains the
   analyzer in both the full and the incremental tier.
4. MUST verify that every other gate step keeps its name, command and order, and
   that the CI workflow is unchanged.
5. MUST verify that no dependency, analyzer configuration file or suppression
   mechanism was added.
6. MUST verify, against the finding Spec 0123 owns and the analyzer's report
   over this module, that the prerequisite this composition waited on is
   satisfied.
