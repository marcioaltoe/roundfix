...
- `grep -q 'archived Spec artifact' skills/owned_skill_edit_repocontract_test.go && ! grep -q '_archived/specs' skills/owned_skill_edit_repocontract_test.go` — expected: exits 0, proving the byte-identity assertion survives and neither old literal does. The two clauses are one command because a "still present" check passes on an unchanged tree and proves nothing until it is paired with a "changed" check.
