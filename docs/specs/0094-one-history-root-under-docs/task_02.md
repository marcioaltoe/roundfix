...
- `grep -q '"docs/history/specs"' internal/spec/archive.go && grep -q '"docs/history/findings"' internal/spec/archive.go && grep -q '"docs/history/adr"' internal/spec/archive.go && grep -q '"docs/history/backlog"' internal/spec/archive.go && grep -q '"docs/history/reviews"' internal/spec/archive.go` — expected: exits 0, proving all five answers moved rather than only the ones this Task's tests read.
- `test -d docs/history/specs && test -d docs/history/findings && test ! -e _archived` — expected: exits 0, proving the bytes moved rather than only the resolver's answer.
- `! grep -n '"_reviews"' internal/config/config.go` — expected: exits 0, proving the orphan review root lost its underscore.
- `go test -count=1 ./internal/config -run 'ReviewRoot|ReviewArtifactRoot' -v > /tmp/0094-task-02c.log 2>&1; s=$?; grep -q '^--- PASS: .*Review' /tmp/0094-task-02c.log || { cat /tmp/0094-task-02c.log; exit 1; }; exit $s` — expected: exits 0 and the log names the passing resolution tests, including the one proving no root resolves into history.
- `grep -q 'Re-recorded because' internal/docscontract/testdata/corpus-golden.json` — expected: exits 0.
- `go test -count=1 ./internal/spec -run ArchiveLayoutCharacterization -v > /tmp/0094-task-02.log 2>&1; s=$?; grep -q '^--- PASS: TestArchiveLayoutCharacterization' /tmp/0094-task-02.log || { cat /tmp/0094-task-02.log; exit 1; }; exit $s` — expected: exits 0 and the log names the passing characterization tests.
- `go test -count=1 ./internal/speccheck > /tmp/0094-task-02b.log 2>&1 || { cat /tmp/0094-task-02b.log; exit 1; }` — expected: exits 0, proving the consistency detectors still resolve retired references after the move. Scoped to the one package whose detectors read the resolver, not a repository-wide sweep.
