...
2. MUST verify that a Task declaring independent Verification hands every
   failure to its one repair turn, and that an undeclared Task still stops at
   its first failure.
3. MUST verify that, with the repository command red, a Task named in
   `precondition_repairs` enters and settles only when the gate passes, an
   unnamed Task stays blocked, and a named Task lacking the verbatim command is
   refused before the Run.
4. MUST verify that requested full access on a runtime or adapter that cannot
   honour it fails readiness and creates no Run, and that a supported mode is
   recorded as the effective policy.
5. MUST verify that ADR-0159 and ADR-0160 are accepted and the configuration
   guide states the readiness refusal.
