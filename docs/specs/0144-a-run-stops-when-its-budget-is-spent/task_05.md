...
2. MUST verify that a Run whose configured maximum expires ends, cancels what the
   Daemon owns, and settles `BudgetExceeded` with a reason naming the maximum and
   the elapsed time.
3. MUST verify that a Run with the budget disabled settles exactly as it does
   today.
4. MUST verify that carry-forward accepts a `BudgetExceeded` Run with proved
   Tasks, refuses one whose Tasks cannot be proved, and still refuses every
   outcome outside the accepted set.
5. MUST verify that the Roundfix skill and its mirror state the bound, the
   outcome and the three accepted carry-forward outcomes, and that the two copies
   are identical.
6. MUST verify, against the watch loop and the Run Database vocabulary, that this
   Spec applies an existing rule and an existing outcome rather than new ones.
