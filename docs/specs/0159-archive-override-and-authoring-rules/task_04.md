...
2. MUST verify through the built binary that an authorized override archives a
   Spec with failed QA, stamps approval, reason, QA outcome and revision, and
   leaves the QA Task and report byte-identical.
3. MUST verify that an override is refused with a non-QA Task pending, with QA
   already qualifying, and without approval or reason, and that archive without
   the flag is unchanged.
4. MUST verify that Spec check reports `SC-ORDINAL-CLAIMED` for a number held on
   the tree and for one claimed by two active Specs, and accepts distinct numbers
   and a fulfilled claim.
5. MUST verify that the settlement section is identical across the three skills
   and their mirrors, and that the task-writing skill names every declaration.
...
ADR-0091; ADR-0093; ADR-0096; ADR-0097; ADR-0104; ADR-0117; ADR-0130; ADR-0154;
ADR-0155; ADR-0156.
...
- Dependencies not completed: `task_05`, `task_06`
