...
- Approved on 2026-09-09: the Spec-contained `_authorization.md` record as canonical placement, over the bounded governed paths in Project Constraints. The initial approval covered twenty-one paths; the 2026-09-09 amendment raised it to twenty-three by adding `internal/suiteguardcontract/regeneration.go` and its test, and the 2026-09-10 amendment raised it to the twenty-five listed there by adding `internal/speccheck/mechanical_test.go` and `internal/speccheck/coherence.go`. Project Constraints is the operative list.
