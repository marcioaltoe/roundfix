...
2. MUST verify that the Codex path runs a reviewer session and writes a record
   naming repository, base and head.
3. MUST verify that `none` performs no reviewer call and no readiness probe,
   records an omission and exits zero.
4. MUST verify that a runtime failure, a timeout and unreadable output each exit
   2 with the reason named, and that none of them records a pass or an omission.
