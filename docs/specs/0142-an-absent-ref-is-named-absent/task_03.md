...
2. MUST verify that a short name matching no ref is reported as absent and that
   one matching several refs is still reported as ambiguous.
3. MUST verify that a Run Branch set whose recorded target branch is absent
   preserves every candidate with a reason naming that branch.
4. MUST verify that a set whose target branch exists reaches the outcome it
   reaches today, and that nothing new is released.
5. MUST verify, against the finding this Spec adopted, that the shape it records
   is the shape the new reason replaces.
6. MUST verify that this Spec's own artifacts satisfy the promise rule: its
...
- [ ] Replay the adopted finding as the outside-evidence row.
...
`_techspec.md` → Testing Approach 1-4; API Contracts 1-3; ADR-0155.
