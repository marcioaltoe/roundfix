...
2. MUST verify that a command failing in both the first run and the exclusive
   retry reaches the repair turn once, and one passing on retry does not.
3. MUST verify that a completed repair Task without the verbatim command does
