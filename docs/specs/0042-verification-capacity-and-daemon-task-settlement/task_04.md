...
- [ ] Add typed exit-75 classification at the real process boundary.
- [ ] Add Task-scoped one-retry state without renumbering attempts.
- [ ] Run the retry under whole-gate exclusive acquisition.
- [ ] Preserve separate initial and retry diagnostics.
- [ ] Publish retry lifecycle and exhaustion evidence.
- [ ] Cover repair interaction, repeated signals, cancellation, and race behavior.

## Acceptance Criteria

- [ ] A real shell command exiting `75` returns the typed temporary failure and
      retains its combined diagnostics; exit `1` remains deterministic.
- [ ] Two normal attempts that could overlap are both drained before an
      exclusive retry begins, and no later shared attempt bypasses it.
- [ ] A temporary failure followed by a passing exclusive retry settles the
      Task completed with zero Agent repair turns.
- [ ] A temporary failure followed by a deterministic exclusive-retry failure
      receives the one existing Agent repair, then follows numbered attempt `2`.
- [ ] A repeated exit `75`, including one occurring after Agent repair when the
      retry budget was already used, settles failed with no second retry.
- [ ] Initial and retry diagnostic files both remain inspectable and no retry
      artifact overwrites an attempt artifact.
- [ ] Event payloads distinguish numbered attempt, retry identity, mode,
      classification, availability, and final verdict without unbounded log text.
- [ ] Output containing known timeout/listener phrases with a non-`75` exit
