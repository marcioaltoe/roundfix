...
3. MUST limit repository mutations to
   `.agents/skills/roundfix/SKILL.md`, `skills/roundfix/SKILL.md`, and this
   `task_03.md` file.
...
5. MUST leave every external skill, lock file, recommendation file, manifest,
   code file, and user-documentation file unchanged.
...
- `rtk git status --porcelain | rtk awk '{path=substr($0,4); if (path != ".agents/skills/roundfix/SKILL.md" && path != "skills/roundfix/SKILL.md" && path != "docs/specs/0036-doctor-skill-readiness/task_03.md") {print; bad=1}} END {exit bad}'`
