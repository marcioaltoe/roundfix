...
def qa_16(s: Scenario) -> None:
    ...
    s.check("optional skill cleanup reporting" in audit_help.stdout.lower(), "audit help truthfully describes implemented extra-skill report")
    s.check("canonical setup drift" in audit_help.stdout.lower(), "audit help truthfully describes implemented setup drift check")
    result = s.run(["--repo", str(s.fixture), "--format", "json"])
    payload = parse_json(result)
