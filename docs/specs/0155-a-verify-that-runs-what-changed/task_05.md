...
2. MUST verify that every package and every `internal/cli` test is selected by
   exactly one set, and that the partition contract fails its negative controls.
3. MUST verify that a core-only change selects no Baseline-set test, a
   Baseline-only change selects no core-set test, and module files select both.
4. MUST verify that an unresolvable base selects both sets.
5. MUST verify that `make verify` still runs the complete suite, and that
   `make verify-changed` runs to completion on a core-only change.
6. MUST verify that `.roundfixrc.yml` and the pull request job name
   `make verify-changed`, and that `main` and release name `make verify`.
