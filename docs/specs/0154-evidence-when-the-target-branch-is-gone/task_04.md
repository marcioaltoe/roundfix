...
2. MUST verify that a Run squash-merged into the default branch, with its target
   branch deleted, is classified releasable and names the evidence.
3. MUST verify that the same Run without that evidence stays preserved, and that
   an unresolvable default branch and evidence naming another Spec each preserve
   too.
4. MUST verify that a dirty Worktree preserves regardless of evidence.
5. MUST verify that every classification for a present target branch is
   unchanged.
6. MUST verify that each preserved reason names the proof that was missing.
7. MUST verify that the shipped skill and its mirror describe the fallback, and
