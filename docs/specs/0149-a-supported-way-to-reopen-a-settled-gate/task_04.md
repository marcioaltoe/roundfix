...
2. MUST verify that a Spec whose QA Task is completed above a dependency that is
   not completed is reopened by the command, and that the Spec loads afterwards
   where it refused before.
3. MUST verify that the command refuses when the QA Task is not completed, and
   when every dependency is completed, naming which condition failed.
4. MUST verify that a refused reopen leaves the Spec directory byte-identical.
5. MUST verify that a successful reopen leaves the QA Report's bytes unchanged
   and keeps the QA Task's prior Result, with the invalidation recorded beside
   it.
6. MUST verify that the command creates no Run, writes no Run Event Journal
   entry, commits nothing and pushes nothing.
7. MUST verify that the shipped skill and its mirror name the command and its
   refusals, and that every Task commit stayed inside the approved authority
   record.
