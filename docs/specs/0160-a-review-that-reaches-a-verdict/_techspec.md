...
5. Terminal QA (depends on: 1, 2, 3, 4, 6, 7, 8, 9, 10, 11).
...
9. A no-findings verdict must account for the whole answer (depends on: 8).
10. Describe the whole-answer verdict contract (depends on: 9).
11. A pass is the whole answer (depends on: 10).
