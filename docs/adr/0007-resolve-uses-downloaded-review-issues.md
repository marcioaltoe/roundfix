...
This means Roundfix does not need `fetch` to upsert Review Issue artifacts in place. An automatic `fetch` may reuse an existing matching Round for the same HEAD and fingerprint set; when a new payload creates repeated findings in newer Round directories, `resolve` remains the boundary that decides which occurrence should create Agent work.
