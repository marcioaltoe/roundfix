...
With automatic Round selection, fetch reuses an existing Round when the same HEAD already has the same Review Issue fingerprints. If the fetched payload is new, Roundfix writes the next available Round instead of mutating an earlier one. If the user selects an explicit Round that already exists, fetch fails instead of overwriting local artifacts.
