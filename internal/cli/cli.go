...
supported acpx version, the effective adapter, required Agent Selection
Profiles, the Repository Skill Set, and codex runtime hygiene. The aggregate
profiles: line exact-proves every distinct tuple. The skills: line compares
Roundfix-owned artifacts with the running binary and external artifacts with
skills-lock.json. Each failure reports its next action. Doctor is offline,
