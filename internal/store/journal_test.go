// Suite: Run Event Journal storage maintenance
// Invariant: maintenance preserves live Run state and mutates database bytes only behind its explicit safety guards.
// Boundary IN: the real temporary SQLite Run Database and its on-disk files.
// Boundary OUT: GC Command parsing and reports, owned by internal/cli/gc_test.go.
package store
